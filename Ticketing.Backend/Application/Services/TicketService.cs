...
public class TicketService : ITicketService
{
    ...
    public async Task<TicketResponse?> CreateTicketAsync(Guid userId, TicketCreateRequest request)
    {
        ...
        // Auto-assign technician if smart assignment is enabled
        var settings = await _systemSettingsService.GetSystemSettingsAsync();
        if (settings.AutoAssignEnabled && ticket.TechnicianId == null)
        {
            await _smartAssignmentService.AssignTechnicianToTicketAsync(ticket.Id);
        }
    ...
    public async Task<TicketResponse?> AssignTicketAsync(Guid id, Guid technicianId)
    {
        ...
        var isActive = await _technicianService.IsTechnicianActiveAsync(technicianId);
        if (!isActive)
        {
        ...
        ticket.AssignedToUserId = null; // Clear old user assignment if exists
    ...
    private static TicketResponse MapToResponse(Ticket ticket) => new()
    {
        ...
        AssignedToName = ticket.Technician?.FullName ?? ticket.AssignedToUser?.FullName,
        AssignedToEmail = ticket.Technician?.Email ?? ticket.AssignedToUser?.Email,
        AssignedToPhoneNumber = ticket.Technician?.Phone ?? ticket.AssignedToUser?.PhoneNumber,
        AssignedTechnicianName = ticket.Technician?.FullName ?? ticket.AssignedToUser?.FullName,
    ...
    public async Task<IEnumerable<TicketCalendarResponse>> GetCalendarTicketsAsync(DateTime startDate, DateTime endDate)
    {
        ...
        return tickets.Select(t => new TicketCalendarResponse
        {
            ...
            AssignedTechnicianName = t.Technician?.FullName ?? t.AssignedToUser?.FullName,
