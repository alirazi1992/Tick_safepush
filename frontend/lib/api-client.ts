...
import { normalizeBaseUrl, joinApi, getEffectiveApiBaseUrl } from "./url";
...
function getBackendUrlCandidates(): string[] {
  ...
  // Single source of truth: environment variable if set, otherwise backend default
...
async function detectApiBaseUrl(): Promise<string> {
  ...
  // If detection is already in progress, use default to avoid loops
  ...
  try {
    // Check cache first (no health probing to avoid extra requests)
    ...
    const selected = candidates[0] || "http://localhost:5000";
...
export async function getApiBaseUrl(): Promise<string> {
  ...
  // GUARD: Ensure we always return an absolute URL
  if (!API_BASE_URL || !API_BASE_URL.startsWith("http://") && !API_BASE_URL.startsWith("https://")) {
    const defaultUrl = "http://localhost:5000";
    console.error("[api-client] ERROR: getApiBaseUrl returned invalid URL:", API_BASE_URL, "- using default:", defaultUrl);
    API_BASE_URL = defaultUrl;
  }

  // Debug log on startup (dev only)
  if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
    console.log("[api-client] Resolved API Base URL:", API_BASE_URL);
    console.log("[api-client] NEXT_PUBLIC_API_BASE_URL:", process.env.NEXT_PUBLIC_API_BASE_URL || "(not set, using default: http://localhost:5000)");
...
export async function apiRequest<TResponse>(
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 && token && token !== COOKIE_SESSION_TOKEN && typeof window !== "undefined") {
