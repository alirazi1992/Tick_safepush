...
export function EnhancedAutoAssignment({
  ...
  const loadStatus = async () => {
    ...
    try {
      ...
      console.error("Failed to load smart assignment status:", error)
      toast({
        title: "خطا در بارگذاری وضعیت",
  ...
  const handleToggle = async (enabled: boolean) => {
    ...
    try {
      ...
      toast({
        title: enabled ? "سیستم هوشمند فعال شد" : "سیستم هوشمند غیرفعال شد",
        description: enabled
          ? "تیکت‌های جدید به صورت خودکار تخصیص داده می‌شوند"
          : "تیکت‌های جدید به صورت خودکار تخصیص داده نمی‌شوند",
      })
    } catch (error: any) {
      console.error("Failed to update smart assignment status:", error)
      toast({
  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      // Refresh tickets if onTicketUpdate is available
      if (onTicketUpdate) {
        // Trigger a refresh by calling onTicketUpdate with a dummy update
        // The parent component should handle refreshing the ticket list
        window.location.reload() // Simple approach - refresh the page
      }
    } catch (error: any) {
      console.error("Failed to run smart assignment:", error)
      toast({
        title: "خطا در اجرای تخصیص خودکار",
        description: error?.message || "لطفاً دوباره تلاش کنید",
        variant: "destructive",
      })
    } finally {
                ...
                <>
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="smart-assignment" className="text-sm font-medium">
                      {isEnabled ? "غیرفعال‌سازی سیستم هوشمند" : "فعال‌سازی سیستم هوشمند"}
                    </Label>
                  </div>
                    ...
                    >
                      {runningAssignment ? (
                        ...
                        <>
                          ...
                          تخصیص خودکار تیکت‌های بدون تکنسین
                        </>
                      )}
                    </Button>
