...
export function TechnicianDashboard({
  ...
  const filteredTickets = useMemo(() => {
    return sortedTickets.filter((ticket) => {
      ...
      const effectiveStatus = getEffectiveStatus(ticket.displayStatus ?? ticket.status, "engineer")
  ...
  const openTickets = sortedTickets.filter(
    (ticket) => getEffectiveStatus(ticket.displayStatus ?? ticket.status, "engineer") === "Open"
  )
  const inProgressTickets = sortedTickets.filter(
    (ticket) => getEffectiveStatus(ticket.displayStatus ?? ticket.status, "engineer") === "InProgress"
  )
  const redoTickets = sortedTickets.filter(
    (ticket) => getEffectiveStatus(ticket.displayStatus ?? ticket.status, "engineer") === "Redo"
  )
  const unreadTickets = sortedTickets.filter((ticket) => ticket.isUnseen)
  const solvedTickets = sortedTickets.filter(
    (ticket) => getEffectiveStatus(ticket.displayStatus ?? ticket.status, "engineer") === "Solved"
  )
                    ...
                    <Card key={ticket.id} className="cursor-pointer" onClick={() => handleViewTicket(ticket.id)}>
                      <CardContent className="p-3">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          <Badge className={getTicketStatusColor(ticket.displayStatus ?? ticket.status, "engineer")}>
                            {getTicketStatusLabel(ticket.displayStatus ?? ticket.status, "engineer")}
                          </Badge>
