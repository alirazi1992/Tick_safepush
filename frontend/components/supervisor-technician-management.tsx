...
export function SupervisorTechnicianManagement() {
  ...
  const canUseSupervisorEndpoints =
    user?.role === "admin" || (user?.role === "engineer" && user?.isSupervisor);

  const loadList = async () => {
    if (!token || !canUseSupervisorEndpoints) return
    try {
      setLoading(true)
      setLoadError(null)
                      ...
                      {summary.archiveTickets.map((ticket, index) => {
                        ...
                        return (
                          <TableRow key={key}>
                            ...
                            <TableCell>
                              <Badge variant="outline">{getTicketStatusLabel((ticket.displayStatus ?? ticket.status) as TicketStatus, "engineer")}</Badge>
                            </TableCell>
                          </TableRow>
                        );
                      })}
