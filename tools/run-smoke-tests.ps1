...
Write-Host ("-" * 100)
$results | Format-Table -Property @{Label="Status"; Expression={if ($_.Status -eq "PASS") {"✅"} elseif ($_.Status -eq "FAIL") {"❌"} else {"⏭️"}}}, Test, ExpectedStatus, ActualStatus, Description -AutoSize
Write-Host ("-" * 100)
Write-Host ""
...
- ✅ **Passed**: $passed
- ❌ **Failed**: $failed
$(if ($skipped -gt 0) { "- ⏭️ **Skipped**: $skipped`n" }) - **Total**: $($results.Count)
...
$($results | ForEach-Object { 
    $status = if ($_.Status -eq "PASS") { "✅ PASS" } elseif ($_.Status -eq "FAIL") { "❌ FAIL" } else { "⏭️ SKIP" }
...
---
*Generated by automated smoke test script*
